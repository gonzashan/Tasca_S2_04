...
/* 4) Escriu una consulta per mostrar el restaurant_id, name, borough i zip code, però exclou el 
...
Operadores relacionales $gt: greater than
...
i la seva qualificació és superior a 70 i longitud inferior a -65.754168. $ne: not equal */
db.restaurants.find( { $and : [ {"cuisine" : {$ne :"American "}}, {"grades.score" : {$gt : 70}}, 
	{"address.coord" : {$lt : -65.754168 } } ] } );

/* 12) Escriu una consulta per trobar els restaurants que no preparen cap cuisine de 'American' i van 
aconseguir un marcador més de 70 i localitzat en la longitud menys que -65.754168. 
Nota: Fes aquesta consulta sense utilitzar $and operador. $ne - not equal - distinto */
db.restaurants.find( { "cuisine" : {$ne :"American "}, "grades.score" : {$gt : 70}, "address.coord" : {$lt : -65.754168 } } );

/* 13) Escriu una consulta per trobar els restaurants que no preparen cap cuisine de 'American' i van obtenir un 
punt de grau 'A' no pertany a Brooklyn. S'ha de mostrar el document segons la cuisine en ordre descendent. $ne - not equal - distinto */
...
/* 15) Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants
 que contenen 'ces' com les últimes tres lletres en el seu nom. */ 
db.restaurants.find( { "name" : /ces$/}, { "restaurant_id" : 1, "name" : 1, "borough" : 1, "cuisine" :1 } );

/* 16) Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants
 que contenen 'Reg' com tres lletres en algun lloc en el seu nom. Oju. sensible a las mayúsculas*/
...
/* 18) Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que pertanyen 
a Staten Island o Queens o Bronx o Brooklyn. */
...
peix excepte 'American' i 'Chinees' o el name del restaurant comença amb lletres 'Wil'. $ne - not equal - distinto */
db.restaurants.find( { $or : [ { name: /^Wil/ }, { "$and" : [ { "cuisine" : {$ne : "American"} } , {"cuisine" : {$ne :"Chinees"}} ] } ] }, ç
	{ "restaurant_id" : 1, "name" : 1, "borough" : 1, "cuisine" : 1 } );
...
/* 23) Escriu una consulta per trobar el restaurant_id, name i grades per a aquells restaurants on el 
2n element de varietat de graus conté un grau de "A" i marcador 9 sobre un ISODate "2014-08-11T00:00:00Z".*/ 
...
/* 28) Escriu una consulta per saber totes les direccions que no contenen el carre.*/
...
restaurants que contenen 'Mad' com primeres tres lletres del seu nom.*/
